...
static PQUERY_KEY LoadMultipleBlobs(LPBYTE pbLineBegin, LPBYTE pbLineEnd, DWORD * pdwBlobCount)
{
    ...
    if(dwBlobCount != 0)
    {
        ...
        pBlobArray = CASC_TEMP_ALLOC(QUERY_KEY, dwBlobCount);
        if(pBlobArray != NULL)
        {
            // Zero the blob array
            ...
            pbBuffer = CASC_TEMP_ALLOC(BYTE, dwBlobCount * MAX_CASC_KEY_LENGTH);
            if(pbBuffer != NULL)
            {
                // Zero the buffer
                ...
                // Free the buffer
                CASC_TEMP_FREE(pbBuffer);
            }

            // Free the array of blobs
            CASC_TEMP_FREE(pBlobArray);
            pBlobArray = NULL;
        }
